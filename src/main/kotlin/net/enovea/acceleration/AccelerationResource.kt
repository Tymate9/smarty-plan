...
class AccelerationResource(private val ggDiagramService: GGDiagramService) {
    ...
    @GET
    @Path("/{id}/gg-diagram")
    fun computeGGDiagram(@PathParam("id") deviceId: Int, @QueryParam("beginDate") beginDate: String, @QueryParam("endDate") endDate: String): Response {
        try {
            val ggDiagram = ggDiagramService.computeGGDiagram(deviceId, beginDate, endDate)
